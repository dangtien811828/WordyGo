...
// Unique-per-run prefix — tests không đụng data sẵn có; afterAll cleanup theo prefix này.
// user_refresh_tokens ON DELETE CASCADE → tự dọn theo.
...
describe('POST /api/v1/auth/register', () => {
  it('happy path → 201 with accessToken + refreshToken + user', async () => {
...
describe('GET /api/v1/auth/me', () => {
  ...
  it('with expired token → 401 TOKEN_EXPIRED', async () => {
    // Sign một token đã hết hạn 1 phút trước
...
describe('POST /api/v1/auth/logout & /logout-all', () => {
  ...
  it('logout-all revokes every refresh token for the user', async () => {
    ...
    const reg = await register(email);
    const { accessToken, refreshToken: rt1 } = reg.body.data;
